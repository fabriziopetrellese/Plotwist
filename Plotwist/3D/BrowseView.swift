    ...
    var body: some View{
        VStack{
            ForEach(ModelCategory.allCases, id: \.self) { category in
                ...
                //display only if contains item
                
                if let modelsByCategory = models.get(category: category) {
                    HorizontalGrid(showBrowse: $showBrowse, title: category.lable, items: modelsByCategory)
                }
            }
        }
    }
                ...
                LazyHGrid(rows: gridItemLayout, spacing: 30) {
                    ForEach(0..<items.count) { index in
                        ...
                        ItemButton(model: model) {
                            ...
                            //TODO: call model method to async load  modelentuty
